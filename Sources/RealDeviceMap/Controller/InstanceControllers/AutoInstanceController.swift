...
class AutoInstanceController: InstanceControllerProto {
    ...
    private let ignoreRwForQuest = (ProcessInfo.processInfo.environment["IGNORE_RW_FOR_QUEST"] as NSString).boolValue ?? false
    ...
    func getAccount(mysql: MySQL, uuid: String, encounterTarget: Coord?) throws -> Account? {
        ...
        if let usernane = accounts[uuid] {
            ...
            return try Account.getNewAccount(
                ...
                minLevel: minLevel,
                maxLevel: maxLevel,
                ignoringWarning: ignoreRwForQuest,
                spins: spinLimit,
                noCooldown: true,
    ...
    func accountValid(account: Account) -> Bool {
        return
            ...
            account.isValid(ignoringWarning: ignoreRwForQuest, group: accountGroup) &&
