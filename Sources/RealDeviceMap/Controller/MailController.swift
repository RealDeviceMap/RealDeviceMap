...
class MailController {
    ...
    private static func setupClient() {
        if clientURL == nil || clientURL! == "" || clientPassword == nil || clientPassword! == "" || clientUsername == nil || clientUsername! == "" {
            client = nil
        } else {
            client = SMTPClient(url: clientURL!, username: clientUsername!, password: clientPassword!, requiresTLSUpgrade: true)
        }
    }
    ...
    public func sendConfirmEmail(recipient: Recipient, key: String, completion: @escaping (Int) -> ()) throws {
        ...
        values["email_confirm_title_1"] = Localizer.global.get(value: "email_confirm_title_1", replace: ["name" : recipient.name])
        values["email_confirm_title_2"] = Localizer.global.get(value: "email_confirm_title_2", replace: ["name" : WebReqeustHandler.title])
        ...
        try sendMail(template: .confirmMail, values: values, recipient: recipient, subject: subject, completion: { (code) in
            completion(code)
        })
        
    }
    
    public func sendResetEmail(recipient: Recipient, key: String, completion: @escaping (Int) -> ()) throws {
        ...
        values["footer_html"] = MailController.footerHtml
        values["email_reset_title_1"] = Localizer.global.get(value: "email_reset_title_1", replace: ["name" : recipient.name])
        values["email_reset_title_2"] = Localizer.global.get(value: "email_reset_title_2", replace: ["name" : WebReqeustHandler.title])
        ...
        try sendMail(template: .resetPassword, values: values, recipient: recipient, subject: subject, completion: { (code) in
            completion(code)
        })
        
    }
    
    private func sendMail(template: Templates, values: MustacheEvaluationContext.MapType, recipient: Recipient, subject: String, completion: @escaping (Int) -> ()) throws {
    ...
    public func sendMail(text: String, isHtml: Bool, recipient: Recipient, subject: String, completion: @escaping (Int) -> ()) throws {
