...
extension Polygon {
    ...
    func getS2CellIDs(minLevel: Int, maxLevel: Int, maxCells: Int) -> [S2CellId] {
        ...
        let region = S2LatLngRect(
            lo: S2LatLng(lat: S1Angle(degrees: bbox!.southEast.latitude), lng: S1Angle(degrees: bbox!.northWest.longitude)),
            hi: S2LatLng(lat: S1Angle(degrees: bbox!.northWest.latitude), lng: S1Angle(degrees: bbox!.southEast.longitude))
        )
