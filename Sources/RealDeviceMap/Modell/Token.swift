...
class Token {
    ...
    public static func create(mysql: MySQL?=nil, type: TokenType,
                              username: String, validSeconds: UInt32) throws -> Token {
    ...
    public static func validate(mysql: MySQL?=nil, token: String, username: String, type: TokenType) throws -> Bool {
        guard let token = try Token.get(mysql: mysql, token: token),
              token.username == username, token.type == type else {
            return false
        }
        return true
    }
