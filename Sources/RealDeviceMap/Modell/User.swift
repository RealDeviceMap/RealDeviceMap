...
class User {
    ...
    private init(username: String, email: String, passwordHash: String, emailVerified: Bool=false,
                 discordId: UInt64?=nil, groupName: String) {
    ...
    public static func register(mysql: MySQL?=nil, username: String, email: String, password: String,
                                groupName: String) throws -> User {
    ...
    private static func checkUsernameValid(username: String) -> Bool {
        ...
        if username =~ regex {
            if !username.contains(string: "  ") && !username.contains(string: "__") &&
    ...
    private static func login(mysql: MySQL?=nil, username: String? = nil, email: String? = nil,
                              password: String, host: String) throws -> User {
    ...
    public static func get(mysql: MySQL?=nil, username: String? = nil, email: String? = nil) throws -> User? {
        ...
        if results.numRows == 0 {
            ...
            return User(username: username, email: email, passwordHash: passwordHash,
                        emailVerified: emailVerified, discordId: discordId, groupName: groupName)
        }
    }
    ...
    public static func getAll(onlyDiscord: Bool=false, mysql: MySQL?=nil) throws -> [User] {
        ...
        while let result = results.next() {
            ...
            users.append(User(username: username, email: email, passwordHash: passwordHash,
                              emailVerified: emailVerified, discordId: discordId, groupName: groupName))
        }
        return users
    }
