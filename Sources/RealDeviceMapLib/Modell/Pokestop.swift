...
public class Pokestop: JSONConvertibleObject, WebHookEvent, Hashable {
    ...
    public func addQuest(title: String, questData: QuestProto) {
    ...
    public static func getAll(
        ...
        let sql = """
            SELECT id, lat, lon, name, url, enabled, lure_expire_timestamp, last_modified_timestamp, updated,
                   ...
                   CAST(quest_rewards AS CHAR), quest_template, quest_title, cell_id, lure_id, pokestop_display,
        ...
        while let result = results.next() {
            ...
            let cellId = result[16] as? UInt64
            let lureId: Int16?
            if showLures {
                lureId = result[17] as? Int16
            } else {
                lureId = nil
            }
            ...
            if showInvasions {
                pokestopDisplay = result[18] as? UInt16
                incidentExpireTimestamp = result[19] as? UInt32
                gruntType = result[20] as? UInt16
            } else {
                pokestopDisplay = nil
                incidentExpireTimestamp = nil
                gruntType = nil
            }
            let sponsorId = result[21] as? UInt16
            let arScanEligible = (result[22] as? UInt8)?.toBool()
    ...
    public static func getIn(mysql: MySQL?=nil, ids: [String]) throws -> [Pokestop] {
        ...
        while let result = results.next() {
            ...
            let cellId = result[16] as? UInt64
            let lureId = result[17] as? Int16
            let pokestopDisplay = result[18] as? UInt16
            let incidentExpireTimestamp = result[19] as? UInt32
            let gruntType = result[20] as? UInt16
            let sponsorId = result[21] as? UInt16
            let arScanEligible = (result[22] as? UInt8)?.toBool()
