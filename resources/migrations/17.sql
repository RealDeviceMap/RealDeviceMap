...
FOR EACH ROW BEGIN
  IF ((OLD.quest_type IS NULL OR OLD.quest_type = 0) AND (NEW.quest_type IS NOT NULL AND NEW.quest_type != 0)) THEN
    INSERT INTO quest_stats (reward_type, pokemon_id, item_id, level, count, date)
    VALUES
      (NEW.quest_reward_type, NEW.quest_pokemon_id, NEW.quest_item_id, 1, DATE(FROM_UNIXTIME(NEW.quest_timestamp)))
